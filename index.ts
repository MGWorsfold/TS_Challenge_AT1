...
const colourInput = document.getElementById('colourSelect') as HTMLSelectElement;
const bodyPartInput = document.getElementById('bodyPartSelect') as HTMLSelectElement;
...
function spinBtnHandler(time: number, interval: number) {
  ...
  //  randomly select colour from array
...
function stopSpinners() {
  ...
  // set colourDiv and bodyPartP to the randomly spun results
...
function addToHistory() {
  spinHistoryArray.push(
    ...
    let colourCell: HTMLTableCellElement = <HTMLTableCellElement>(newRow.insertCell()
    );
...
function statsBtnHandler() {
  ...
  let selectedColour: Colours = ColoursHelper.get(colourInput.options[colourInput.selectedIndex].text
  );
